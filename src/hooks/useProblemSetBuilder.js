...
export const useProblemSetBuilder = () => {
    ...
    (config) => {
      ...
      if (filteredProblems.length === 0) {
        return {
          ...
          problems: [],
          warnings: [
            'No problems found for selected topics. Please choose different topics or select "All Topics".',
          ],
      ...
      Object.entries(counts).forEach(([difficulty, count]) => {
        ...
        if (available.length === 0) {
          warnings.push(
            `No ${difficulty} problems available for selected topics.`,
          );
          return;
        }
      ...
      const actualCounts = {
        ...
        Medium: selectedProblems.filter((p) => p.difficulty === "Medium")
          .length,
   ...
   */
  const getAvailableStats = useCallback(
    (topics = []) => {
      let filteredProblems = allProblems;
