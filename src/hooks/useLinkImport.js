...
export const useLinkImport = () => {
    ...
    const importFromLink = useCallback(async (url) => {
        if (!url) return;
        setImporting(true);
        ...
        try {
            ...
            if (url.includes('/list/')) {
                ...
                // Study plan format
                const planSlug = url.split('/studyplan/')[1]?.split('/')[0]?.split('?')[0];
                ...
                throw new Error('Unsupported URL format. Please use a list, study plan, or tag URL.');
            }
            ...
            alert(`Successfully imported ${addedCount} new problems from the link!`);
